    ...
    return (
        <div>
            ...
            <form onSubmit={savePlace}>
                {preInput('Title', 'Title for Your Place, Short and Sweet!')}
                <input type="text" placeholder="Title - eg: Lovely Apartment on the Hills" value={title} onChange={ev => setTitle(ev.target.value)} />
                ...
                {preInput("Check-In and Check-Out Times, Max # of Guests", "Add Check-In and Check-Out Times. Remember to Have Some Window for Clearning in Between Guests")}
                <div className="mt-4 grid gap-2 grid-cols-2 md:grid-cols-4">
                    <div>
                        <h3 className="mt-2 -mb-1">Check-In Time</h3>
                        <input type="text" placeholder="Eg - 14" value={checkIn} onChange={ev => setCheckIn(ev.target.value)} />
                    </div>
                    <div>
                        <h3 className="mt-2 -mb-1">Check-Out Time</h3>
                        <input type="text" placeholder="Eg - 08" value={checkOut} onChange={ev => setCheckOut(ev.target.value)} />
                    </div>
                    <div>
                        <h3 className="mt-2 -mb-1">Max # of Guests</h3>
                        <input type="number" placeholder="Eg - 5" value={maxGuests} onChange={ev => setMaxGuests(ev.target.value)} />
                    </div>
                    <div>
                        <h3 className="mt-2 -mb-1">Price per Night</h3>
                        <input type="number" placeholder="Eg - 5" value={price} onChange={ev => setPrice(ev.target.value)} />
                    </div>
                </div>
